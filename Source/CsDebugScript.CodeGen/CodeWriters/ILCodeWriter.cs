...
namespace CsDebugScript.CodeGen.CodeWriters
{
    ...
    internal class ILCodeWriter : DotNetCodeWriter
    {
        ...
        public override byte[] GenerateBinary(IEnumerable<UserType> userTypes)
        {
            ...
            // TODO: This should be saved into byte stream instead file on disk.
            assemblyBuilder.Save("ILTest.dll");
            Console.WriteLine($"(IL emitting time: {sw.Elapsed.TotalSeconds}s)");
            return null;
#else
            throw new NotImplementedException();
#endif
            ...
            if (dataField.Type is BasicTypeInstance basicType)
            {
                ...
                if (!string.IsNullOrEmpty(basicTypeName))
                {
                    ...
                    if (field.LocationType == LocationType.BitField)
                    {
                        ...
                        // Read function call
                        MethodInfo readFunction = Defines.UserType_Methods.Where(m => m.Name == readFunctionName && m.GetParameters().Length == 4).Single();
                    ...
                    {
                        // Read function call
                        MethodInfo readFunction = Defines.UserType_Methods.Where(m => m.Name == readFunctionName && m.GetParameters().Length == 2).SingleOrDefault();
                ...
                if (codeArrayType.ElementType is BasicTypeInstance basic)
                {
                    ...
                    if (!string.IsNullOrEmpty(basicTypeName))
                    {
                        ...
                        if (basicTypeName == "Char")
                        {
                            ...
                            // Read function call
                            MethodInfo readFunction = Defines.UserType_Methods.Where(m => m.Name == readFunctionName && m.GetParameters().Length == 4).First();
                        ...
                        {
                            // Read function call
                            MethodInfo readFunction = Defines.UserType_Methods.Where(m => m.Name == readFunctionName && m.GetParameters().Length == 3).First();
        ...
        private void CastVariableToDataFieldType(ILGenerator il, DataFieldUserTypeMember dataField, GeneratedTypes generatedTypes)
        {
            ...
            if (dataField.Type is VariableTypeInstance)
            {
                ...
                il.Emit(OpCodes.Newobj, dataFieldType);
            else
